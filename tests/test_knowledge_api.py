"""API contract tests for knowledge endpoints (currently not exposed)."""
...
def test_knowledge_list_endpoint_is_not_registered_yet() -> None:
    tenant_id = UUID("00000000-0000-0000-0000-000000000001")
    with _client() as client:
        response = client.get(f"/api/v1/tenants/{tenant_id}/knowledge")
    assert response.status_code == 404


def test_knowledge_upload_endpoint_is_not_registered_yet() -> None:
    tenant_id = UUID("00000000-0000-0000-0000-000000000001")
    with _client() as client:
        response = client.post(f"/api/v1/tenants/{tenant_id}/knowledge")
    assert response.status_code == 404
