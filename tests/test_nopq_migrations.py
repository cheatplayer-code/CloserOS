"""PostgreSQL migration tests for NOPQ AI and knowledge schema."""
...
def _create_isolated_database_url() -> tuple[str, str, str]:
    ...
    database_name = f"closeros_nopq_migration_{uuid.uuid4().hex[:12]}"
...
def test_nopq_migration_revision_is_head() -> None:
    config = build_alembic_config("postgresql+psycopg://local/local@127.0.0.1:5432/local")
    script = ScriptDirectory.from_config(config)
    assert script.get_current_head() == "f6a8c2e4b1d3"


def test_nopq_migration_upgrade_creates_ai_and_knowledge_tables() -> None:
...
def test_nopq_migration_downgrade_and_reupgrade() -> None:
    admin_url, database_name, database_url = _create_isolated_database_url()
    config = build_alembic_config(database_url)
    try:
        command.upgrade(config, "head")
        command.downgrade(config, "d1f3a5c7e9b2")
        engine = create_migration_engine(database_url)
        try:
            downgraded = set(inspect(engine).get_table_names())
        finally:
            engine.dispose()
        assert "tenant_ai_policies" not in downgraded
        assert "knowledge_documents" not in downgraded
        assert "conversation_analysis_runs" not in downgraded
        ...
        try:
            ...
            engine.dispose()
        assert "tenant_ai_policies" in upgraded
        assert "knowledge_chunk_terms" in upgraded
        assert "conversation_findings" in upgraded
    finally:
        _drop_database(admin_url, database_name)
