...
def thread_analysis_deduplication_key(*, thread_id: UUID) -> str:
    return f"thread_analyze_{thread_id}"
    ...
    async def enqueue_for_thread(
        ...
        *,
        ...
        async with uow:
            ...
            if thread is None:
                return None
            from closeros.infrastructure.integrated_unit_of_work import (
                SqlAlchemyIntegratedUnitOfWork,
            )
            ...
            if isinstance(uow, SqlAlchemyIntegratedUnitOfWork):
                message_id = await ProductQueryRepository(uow.session).latest_message_id_for_thread(
                    tenant_id=tenant_id,
            ...
            try:
                await uow.outbox_jobs.enqueue(
                    build_outbox_job(
                        ...
                        deduplication_key=thread_analysis_deduplication_key(thread_id=thread_id),
                        created_at=requested_at,
                    )
                )
                await uow.commit()
